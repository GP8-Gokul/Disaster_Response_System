...
class _DisasterEventsScreenState extends State<DisasterEventsScreen> {
  ...
  Future<List<Map<String, dynamic>>> fetchDisasterEvents() async {
    final response = await http.post(
      Uri.parse('http://10.0.2.2:5000/select'),
                            ...
                            TextButton(
                              onPressed: () {
                                showDialog(
                                  context: context,
                                  builder: (BuildContext context) {
                                  TextEditingController eventTypeController = TextEditingController(text: event['event_type']);
                                  TextEditingController locationController = TextEditingController(text: event['location']);
                                  TextEditingController startDateController = TextEditingController(text: event['start_date']);
                                  TextEditingController endDateController = TextEditingController(text: event['end_date']);
                                  TextEditingController descriptionController = TextEditingController(text: event['description']);

                                  return AlertDialog(
                                    title: Text('Edit ${event['event_name']}'),
                                    content: Column(
                                    mainAxisSize: MainAxisSize.min,
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                    ...
                                    TextButton(
                                      onPressed: () {
                                        ...
                                        http.post(
                                        Uri.parse('http://10.0.2.2:5000/update'),
                                        headers: {'Content-Type': 'application/json'},
                                        body: jsonEncode({
                                          'table': 'disaster_events',
                                          'column': 'event_type',
                                          'value': eventTypeController.text,
