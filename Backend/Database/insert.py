...
def insert_aid_distribution(event_id, resource_id, volunteer_id, quantity_distributed, distribution_date, location):
 cursor=get_cursor()
 cursor.execute(f"insert into aid_distribution values({event_id}, {resource_id}, {volunteer_id}, {quantity_distributed}, {distribution_date}, {location});")
 commit()


def insert_user(username, password_hash, role):
 cursor=get_cursor()
 cursor.execute(f"insert into users values({username}, {password_hash}, {role});")
 commit()


def insert_incident_report(event_id, report_date, description, reported_by):
 cursor=get_cursor()
 cursor.execute(f"insert into incident_reports values({event_id}, {report_date}, {description}, {reported_by});")
 commit()
